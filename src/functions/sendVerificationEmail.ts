/**
 * sendVerificationEmail — fire the email_verification email via freshify-comms.
 *
 * Models the same shape as sendInviteEmail: reads provider config from
 * portal_settings.email (with env fallbacks) and POSTs the rendered
 * email_verification_v1 template to the comms service. Best-effort —
 * a failure does NOT roll back the verification-token mint; an operator can
 * resend or the user can request a new link via /v1/auth/register again.
 *
 * The matching template lives in freshify-comms — slice 5.18f adds it. Until
 * that ships, comms will return 404 unknown_template and we'll record the
 * error in the portal audit log. Token in Mongo remains usable.
 */
...
function buildVerifyLink(token: string): string {
  ...
  return `${portalUrl}/verify-email/${encodeURIComponent(token)}`;
}
...
export async function sendVerificationEmail(
  ...
  if (emailCfg.provider === "none") {
    ctx.logger.info({ email: input.email }, "email_provider_disabled_skipping_verification");
  ...
  const verifyLink = buildVerifyLink(input.token);
  ...
  const payload = {
    ...
    replyTo,
    idempotencyKey: `verify:${input.token.slice(0, 16)}`,
    data: {
      verifyLink,
      appName,
  ...
  try {
    await collections.portalAuditLog(ctx.db).insertOne({
      at: now,
      actorUserId: null,
      event: sendError ? "portal.verification_email_failed" : "portal.verification_email_sent",
      payload: {
        email: input.email,
        ...(messageId ? { messageId } : {}),
    ...
    ctx.logger.warn({ err }, "portal_audit_log insert failed (verification)");
  }

  if (sendError) {
    ctx.logger.warn({ email: input.email, sendError }, "verification_email_send_failed");
    return { ok: false, error: sendError };
  }
  ctx.logger.info({ email: input.email, messageId }, "verification_email_sent");
